...
export function TradingChart({
    ...
    // Store latest candles in a ref to decouple chart initialization from data updates
    ...
    // Update candles ref when prop changes
    useEffect(() => {
        candlesRef.current = candles;
    }, [candles]);

    // Initialize chart (runs only when height or recipe changes)
    useEffect(() => {
        ...
        // Set initial data immediately to avoid flash of empty chart
    ...
    // Data Update Effect - Efficiently update series data without recreating chart
    useEffect(() => {
        if (!chartRef.current || !candleSeriesRef.current) return;

        // This effect runs when candles or config changes.
        if (candles.length > 0) {
